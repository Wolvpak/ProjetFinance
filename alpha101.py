...
#https://github.com/stefan-jansen/machine-learning-for-trading/blob/main/24_alpha_factor_library/03_101_formulaic_alphas.ipynb
    ...
    df['alpha1'] = (rank(ts_argmax(power(df['Close'], 2), 5)).mul(-.5).stack().swaplevel())
    
    #Alpha 002
    s1 = rank(ts_delta(log(df['Volume']), 2))
    s2 = rank((df['Close'] / df['Open']) - 1)
    alpha = -ts_corr(s1, s2, 6)
    df['alpha2'] = alpha.stack('ticker').swaplevel().replace([-np.inf, np.inf], np.nan)
    
    #Alpha 003
    df['alpha3'] = (-ts_corr(rank(df['Open']), rank(df['Volume']), 10) .stack('ticker').swaplevel().replace([-np.inf, np.inf], np.nan))
    
    #Alpha 004
    df['alpha4'] = (-ts_rank(rank(df['Low']), 9).stack('ticker').swaplevel())
    
    #Alpha 005
    df['alpha5'] = (rank(df['Open'].sub(ts_mean(vwap, 10))).mul(rank(df['Close'].sub(vwap)).mul(-1).abs()).stack('ticker').swaplevel())
    
    #Alpha 006
    df['alpha6'] = (-ts_corr(o, v, 10).stack('ticker').swaplevel())
    ...
    #Alpha 011
    df['alpha11'] = (rank(ts_max(vwap.sub(c), 3))
    ...
    #Alpha 018
    df['alpha18'] = (-rank(ts_std(c.sub(o).abs(), 5)
                  .add(c.sub(o))
                  .add(ts_corr(c, o, 10)
                       .replace([-np.inf,
                                 np.inf],
    ...
    #Alpha 019
    df['alpha19'] = (-sign(ts_delta(c, 7) + ts_delta(c, 7))
            .mul(1 + rank(1 + ts_sum(r, 250)))
            .stack('ticker')
            .swaplevel())
    
    #Alpha 020
    df['alpha20'] = (rank(o - ts_lag(h, 1))
    ...
    #Alpha 022
    df['alpha22'] = (ts_delta(ts_corr(h, v, 5)
                     .replace([-np.inf,
                               np.inf],
                              np.nan), 5)
    ...
    #Alpha 026
    df['alpha26'] = (ts_max(ts_corr(ts_rank(v, 5), 
                           ts_rank(h, 5), 5)
                   .replace([-np.inf, np.inf], np.nan), 3)
            .mul(-1)
            .stack('ticker')
    ...
    #Alpha 028
    df['alpha28'] = (scale(ts_corr(adv20, l, 5)
    ...
    #Alpha 029
    df['alpha29'] = (ts_min(rank(rank(scale(log(ts_sum(rank(rank(-rank(ts_delta((c - 1), 5)))), 2))))), 5)
    ...
    #Alpha 030
    close_diff = ts_delta(c, 1)
    df['alpha30'] = (rank(sign(close_diff)
